...
public class BallEffect {
	...
	public BallEffect(float x, float y, float w, float h) {
		this.x = x - w / 2;
		this.y = y - h / 2;
		statetime = 0;
		alive = true;
		if (anim == null)
			anim = new Animation<TextureRegion>(time, TextureRegion.split(Listener.balleffect_texture, IMAGE_SIZE, IMAGE_SIZE)[0]);
	}
